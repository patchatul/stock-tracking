    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm  ">
      <div className="bg-background border border-dark-green rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            onClick={onClose}
            className="text-medium-green hover:text-white hover:bg-medium-green/50 p-1 rounded-lg transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-medium-green text-xs uppercase tracking-wider block mb-1">
                Ticker Symbol
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              >
                {STOCK_TYPES.map((t) => (
                  <option
                    className="bg-dark-green/80 text-black font-semibold"
                    key={t}
                  >
                    {t}
                  </option>
